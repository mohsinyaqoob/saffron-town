...
const PROMISES = [
  {
    ...
    title: "100% Purity",
    description:
      "No fillers, no artificial colour. If you find any adulteration, full refund — no questions asked.",
  },
  {
    icon: (
      <svg
    ...
    description:
      "We sell only the current season's harvest. Zero old stock. Ever.",
  },
  {
    icon: (
      <svg
    ...
    description:
      "GI-tagged Mongra saffron, farm-direct from Pampore. Independent ISO 3632 batch testing is available on request for bulk orders over 1 kg.",
  },
] as const;
...
export function GuaranteeSection() {
  return (
    <section
      className="bg-dark py-16 lg:py-20"
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 sm:gap-6">
          {PROMISES.map((p) => (
            ...
            >
              ...
              <h3 className="font-display text-base font-semibold text-dark-text">
                {p.title}
              </h3>
              <p className="text-sm leading-relaxed text-dark-text-muted font-body">
                {p.description}
              </p>
            </div>
          ))}
