...
export function Header() {
  ...
  return (
    <header className="bg-background sticky top-0 z-50 border-b-2 border-secondary-border/50 shadow-sm shadow-dark/5">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6 lg:px-20">
        ...
        >
          {NAV_LINKS.map((link) => {
            ...
            const isGifting = link.label === "Gifting";
            ...
            return (
              ...
              >
                <button
                  ...
                  aria-haspopup="true"
                  className={cn(
                    "flex items-center gap-0.5 text-sm font-body transition-colors",
                    isOpen ? "text-text-primary" : "text-secondary hover:text-text-primary"
                  )}
                  onClick={() =>
                ...
                {isShop && (
                  <div
                    className={cn(
                      ...
                      !isOpen && "pointer-events-none group-hover:pointer-events-auto"
                    )}
                  >
                    <div className="bg-background-alt border border-secondary-border/30 rounded-lg shadow-xl shadow-dark/10 py-4 w-[680px] max-w-[95vw]">
                      <div className="grid grid-cols-3 gap-6 px-6">
                        {link.items.map((section) => (
                          <div key={section.category} className="space-y-3">
                            <Link
                              href={section.href}
                              className="block font-display font-semibold text-text-primary hover:text-primary transition-colors"
                            >
                              {section.category}
                            </Link>
                            ...
                            <ul className="space-y-1.5 pt-1">
                              {section.items.map((item) => (
                                <li key={item.href}>
                                  <Link
                                    href={item.href}
                                    className="text-sm text-secondary hover:text-text-primary transition-colors block"
                                  >
                                    {item.label}
                                  </Link>
                                </li>
                ...
                {isGifting && (
                  ...
                  >
                    <div className="bg-background-alt border border-secondary-border/30 rounded-lg shadow-xl shadow-dark/10 py-3 w-[280px]">
                      <ul className="space-y-0.5 px-3">
                        {link.items.map((item) => (
                          <li key={item.href}>
                            <Link
                              href={item.href}
                              className="block py-2 px-2 -mx-2 rounded text-sm text-secondary hover:text-text-primary hover:bg-surface-muted/60 transition-colors"
                            >
        ...
        {/* Right side: Hamburger (mobile) + Cart + Favorites */}
      ...
      {/* Backdrop - behind panel, click to close */}
      <button
        type="button"
            ...
            return (
              <div key={link.label} className="space-y-1">
                <button
                  ...
                  className="w-full flex items-center justify-between py-3 px-3 -mx-3 text-base font-body text-secondary hover:text-text-primary hover:bg-surface-muted/50 rounded-lg transition-colors text-left"
                ...
                {isExpanded && (
                  <div className="pl-4 space-y-1 border-l-2 border-secondary-border/30 ml-3">
                    {link.label === "Shop" &&
                      link.items.map((section) => (
                        <div key={section.category} className="py-2">
                          <Link
                            href={section.href}
                            onClick={() => setMobileMenuOpen(false)}
                            className="block font-display font-semibold text-text-primary hover:text-primary py-1"
                          >
                            {section.category}
                          </Link>
                            ...
                            {section.items.map((item) => (
                              <li key={item.href}>
                                <Link
                                  href={item.href}
                                  onClick={() => setMobileMenuOpen(false)}
                                  className="block text-sm text-secondary hover:text-text-primary py-1 pl-2"
                                >
                                  {item.label}
                                </Link>
                              </li>
                    ...
                    {link.label === "Gifting" &&
                      link.items.map((item) => (
                        <Link
                          ...
                          href={item.href}
                          onClick={() => setMobileMenuOpen(false)}
                          className="block py-2 text-sm text-secondary hover:text-text-primary"
                        >
