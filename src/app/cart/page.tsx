                    ...
                    <article key={item.cartItemId} className="flex flex-col sm:flex-row items-center gap-6 p-6 rounded-[2rem] bg-background border border-secondary-border/10 shadow-lg shadow-dark/5">
                        ...
                        <div className="flex-grow flex flex-col items-center sm:items-start text-center sm:text-left">
                            <div className="flex items-center gap-3">
                                <h3 className="font-display text-xl font-bold text-text-primary">{item.name}</h3>
                                <span className="bg-primary/10 text-primary text-xs font-bold px-2 py-0.5 rounded uppercase tracking-widest">{item.variant.size}</span>
                            </div>
                            <span className="text-sm font-body text-secondary mb-4">{item.subtitle}</span>
                            <div className="flex items-center gap-4 border border-secondary-border/30 rounded-full px-4 py-2">
                                <button onClick={() => updateQuantity(item.cartItemId, item.quantity - 1)} className="text-text-muted hover:text-primary transition-colors disabled:opacity-50 font-bold" disabled={item.quantity <= 1}>-</button>
                                <span className="font-bold text-sm tracking-widest">{item.quantity}</span>
                                <button onClick={() => updateQuantity(item.cartItemId, item.quantity + 1)} className="text-text-muted hover:text-primary transition-colors font-bold">+</button>
                            </div>
                        </div>
                        <div className="flex flex-col items-center sm:items-end justify-between h-full min-h-[120px] pb-2">
                            <div className="text-center sm:text-right mb-4">
                                <p className="font-display text-xl font-bold text-text-primary">
                                    {new Intl.NumberFormat("en-IN", { style: "currency", currency: item.currency, maximumFractionDigits: 0 }).format(item.variant.price * item.quantity)}
                                </p>
                                {item.quantity > 1 && (
                                    <p className="text-xs text-text-muted mt-1 font-body">
                                        {new Intl.NumberFormat("en-IN", { style: "currency", currency: item.currency, maximumFractionDigits: 0 }).format(item.variant.price)} each
                                    </p>
                                )}
                            </div>
                            <button
                ...
                <div className="p-8 rounded-[2rem] bg-surface-muted/50 border border-secondary-border/10 shadow-xl shadow-dark/5 sticky top-32">
                    ...
                    <div className="space-y-4 mb-8 text-sm font-body border-b border-secondary-border/10 pb-8">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="font-bold text-text-primary text-xs tracking-widest text-right">Calculated at checkout</span>
                        </div>
                    </div>
                    <div className="flex justify-between items-center mb-10">
                        <span className="font-display text-xl font-bold text-text-primary">Total</span>
                        <span className="font-display text-3xl font-bold text-text-primary">{formattedTotal}</span>
                    </div>
                    <Button size="lg" className="w-full rounded-2xl mb-4 py-5 shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 transition-all">Proceed to Checkout</Button>
